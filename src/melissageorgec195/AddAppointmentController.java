    ...
    void SaveButtonHandler(ActionEvent event) throws IOException {
        ...
        for( Appointment appointment : appointments) {
            ...
            try {
                ...
                a.setContentText("Couldn't insert appointment, overlapping dates.");
        ...
        if (id != 0) {
            try {
                DBConnection.getInstance().InsertAppointment(id, description, type, startsqlts, endsqlts);
            } catch (SQLException ex) {
                ex.printStackTrace();
            }
        } else {
            Alert a = new Alert(Alert.AlertType.INFORMATION);
            a.setContentText("Couldn't insert appointment, not a valid customer.");
            a.setHeaderText(null);
            a.showAndWait();
        }
